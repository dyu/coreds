...
import { $newParamRangeKey } from '../prk';
var Store = (function () {
    ...
    Store.prototype.fetchNewer = function (debounce) {
        ...
        var req = $newParamRangeKey(empty, empty ? this.pageSize * this.multiplier + 1 : (this.desc ? this.pageSize : this.pageSize * this.multiplier), empty ? undefined : (this.desc ? this.list[0].orig['1'] : this.list[this.list.length - 1].orig['1']));
        this._fetchTs = now;
    ...
    Store.prototype.fetchOlder = function (debounce) {
        ...
        var req = $newParamRangeKey(true, this.desc ? this.pageSize * this.multiplier : this.pageSize, this.desc ? this.list[this.list.length - 1].orig['1'] : this.list[0].orig['1']);
        this._fetchTs = now;
    ...
    Store.prototype.fetchUpdate = function (debounce) {
        ...
        var req = $newParamRangeKey(this.desc, Math.min(this.pageSize, this.list.length), this.desc ? incrementKey(pair.orig['1']) : decrementKey(pair.orig['1']));
        this._fetchTs = now;
